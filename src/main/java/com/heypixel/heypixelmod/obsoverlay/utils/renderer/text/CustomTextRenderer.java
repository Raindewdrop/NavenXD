...
public class CustomTextRenderer {
   ...
   public CustomTextRenderer(String name, int size, int from, int to, int textureSize) {
      ...
      // 如果.ttf文件不存在，尝试.otf文件
      if (in == null) {
         in = this.getClass().getResourceAsStream("/assets/heypixel/VcX6svVqmeT8/fonts/" + name + ".otf");
      }
      
      if (in == null) {
         throw new RuntimeException("Font not found: " + name + " (tried both .ttf and .otf)");
      } else {
         byte[] bytes;
         try {
