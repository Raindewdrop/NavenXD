   ...
   public void onMotion(EventMotion e) {
      if (e.getType() == EventType.PRE) {
         ...
         if (this.offhandItems.isCurrentMode("Golden Apple")) {
            ...
            // 优先考虑附魔金苹果
            int enchantedSlot = InventoryUtils.getItemSlot(Items.ENCHANTED_GOLDEN_APPLE);
            if (enchantedSlot != -1 && timer.delay(this.delay.getCurrentValue())) {
               if (offHand.getItem() == Items.ENCHANTED_GOLDEN_APPLE) {
                  ItemStack enchantedAppleStack = (ItemStack)mc.player.getInventory().items.get(enchantedSlot);
                  if (offHand.getCount() + enchantedAppleStack.getCount() <= 64) {
                     if (enchantedSlot < 9) {
                        mc.gameMode.handleInventoryMouseClick(mc.player.inventoryMenu.containerId, enchantedSlot + 36, 0, ClickType.PICKUP, mc.player);
                     } else {
                        mc.gameMode.handleInventoryMouseClick(mc.player.inventoryMenu.containerId, enchantedSlot, 0, ClickType.PICKUP, mc.player);
                     }
                  ...
                  this.swapOffHand(enchantedSlot);
               }
            } else {
               // 如果没有附魔金苹果，则考虑普通金苹果
         ...
         if (this.switchGoldenApple.getCurrentValue() && !this.offhandItems.isCurrentMode("Golden Apple")) {
            ...
            int enchantedSlot = InventoryUtils.getItemSlot(Items.ENCHANTED_GOLDEN_APPLE);
            if (enchantedSlot != -1) {
               this.swapItem((int)(this.goldenAppleSlot.getCurrentValue() - 1.0F), Items.ENCHANTED_GOLDEN_APPLE);
            } else {
               // 如果没有附魔金苹果，则考虑普通金苹果
