...
public class Blink extends Module {
   ...
   private float currentReleaseSpeed = 0.0F;
   private int releaseAccelerationTicks = 0;
   ...
   public void onEnable() {
      ...
      this.currentReleaseSpeed = 0.0F;
      this.releaseAccelerationTicks = 0;
   ...
   public void onMotion(EventMotion e) {
      if (e.getType() == EventType.PRE && mc.player != null) {
         ...
         // 更新当前释放速度（逐渐加速）
         if (this.releaseAccelerationTicks < 20) { // 20 tick的加速时间
            this.releaseAccelerationTicks++;
            // 从0.5倍速度开始，逐渐增加到1倍速度
            this.currentReleaseSpeed = this.releaseSpeed.getCurrentValue() * (0.5F + 0.5F * (this.releaseAccelerationTicks / 20.0F));
         } else {
            this.currentReleaseSpeed = this.releaseSpeed.getCurrentValue();
         }
         ...
         while ((float)this.releasedTicks < this.currentReleaseSpeed && this.shouldReleaseTicks > 0 && !this.packets.isEmpty()) {
            this.releaseTick();
            this.shouldReleaseTicks--;
         }

         while ((float)this.releasedTicks < this.currentReleaseSpeed && this.isPlayerInDanger() && !this.packets.isEmpty()) {
            this.releaseTick();
         }

         while (
            (float)this.releasedTicks < this.currentReleaseSpeed
         ...
         if (this.disabling) {
            while ((float)this.releasedTicks < this.currentReleaseSpeed && !this.packets.isEmpty()) {
               this.releaseTick();
            }
